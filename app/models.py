...
STRUCTURE_COLORS = {"HOPITAL": "#4a6741", "POSTE DE SANTE": "#8faf6d", "CENTRE DE SANTE": "#2b6cb0", "CLINIQUE": "#8e5fd1", "SAPEUR POMPIER": "#c0392b", "GENDARMERIE": "#5b6b5e", "PHARMACIES": "#c98a2c"}
...
EVALUATION_MAX_TOTAL = sum(max_pts for _, _, max_pts, _ in EVALUATION_FIELDS)
class Evaluation(db.Model):
    ...
    commercial = db.relationship("User", foreign_keys=[commercial_id]); evaluator = db.relationship("User", foreign_keys=[evaluator_id])
    __table_args__ = (db.UniqueConstraint("commercial_id", "year", "month", name="uq_evaluation_commercial_year_month"),)
    @property
