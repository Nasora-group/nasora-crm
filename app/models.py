...
STRUCTURE_COLORS = {
    "HOPITAL": "#4a6741", "POSTE DE SANTE": "#8faf6d", "CENTRE DE SANTE": "#2b6cb0",
    "CLINIQUE": "#8e5fd1", "SAPEUR POMPIER": "#c0392b", "GENDARMERIE": "#5b6b5e",
    "PHARMACIES": "#c98a2c",
}
JOURS = ["lundi", "mardi", "mercredi", "jeudi", "vendredi", "samedi", "dimanche"]
JOURS_LABELS = {"lundi": "Lundi", "mardi": "Mardi", "mercredi": "Mercredi", "jeudi": "Jeudi", "vendredi": "Vendredi", "samedi": "Samedi", "dimanche": "Dimanche"}
...
class Planning(db.Model):
    ...
    lundi = db.Column(db.Text, nullable=True); mardi = db.Column(db.Text, nullable=True); mercredi = db.Column(db.Text, nullable=True)
    jeudi = db.Column(db.Text, nullable=True); vendredi = db.Column(db.Text, nullable=True); samedi = db.Column(db.Text, nullable=True); dimanche = db.Column(db.Text, nullable=True)
...
class ProductMixin:
    ...
    default_price = db.Column(db.Numeric(12, 2), nullable=False, default=Decimal("0.00"))
...
class NovaPharmaSale(SaleMixin, db.Model):
    ...
    product = db.relationship("NovaPharmaProduct"); commercial = db.relationship("User")
class GilbertSale(SaleMixin, db.Model):
    ...
    commercial_id = db.Column(db.Integer, db.ForeignKey("user.id"), nullable=False)
    product = db.relationship("GilbertProduct"); commercial = db.relationship("User")
class EricFavreSale(SaleMixin, db.Model):
    ...
    commercial_id = db.Column(db.Integer, db.ForeignKey("user.id"), nullable=False)
    product = db.relationship("EricFavreProduct"); commercial = db.relationship("User")
class TroisCheneSale(SaleMixin, db.Model):
    ...
    commercial_id = db.Column(db.Integer, db.ForeignKey("user.id"), nullable=False)
    product = db.relationship("TroisCheneProduct"); commercial = db.relationship("User")

SUPPLIERS = {
    "nova_pharma": {"label": "Nova Pharma", "division": "nasderm", "product_model": NovaPharmaProduct, "sale_model": NovaPharmaSale, "archived": True},
    "gilbert": {"label": "Gilbert", "division": "nasderm", "product_model": GilbertProduct, "sale_model": GilbertSale, "archived": False},
    "eric_favre": {"label": "Eric Favre", "division": "nasmedic", "product_model": EricFavreProduct, "sale_model": EricFavreSale, "archived": False},
    "trois_chene": {"label": "3 Chênes Pharma", "division": "nasmedic", "product_model": TroisCheneProduct, "sale_model": TroisCheneSale, "archived": False},
}
...
class SalesObjective(db.Model):
    ...
    target_amount = db.Column(db.Numeric(12, 2), nullable=False, default=Decimal("0.00"))
    __table_args__ = (db.UniqueConstraint("division", "year", "month", name="uq_objective_division_year_month"),)
