...
def _sync_professional_from_prospection(prospection):
    ...
    visit=ClientVisit.query.filter_by(client_id=client.id,commercial_id=current_user.id,date=prospection.date,products_presented=products_presented,products_prescribed=products_prescribed,report=report,is_duplicate=False).order_by(ClientVisit.id.desc()).first()
    if visit is None: visit=ClientVisit(client_id=client.id,commercial_id=current_user.id,date=prospection.date,products_presented=products_presented,products_prescribed=products_prescribed,report=report); db.session.add(visit); db.session.flush()
    return client,visit
...
@roles_required("commercial")
def prospections():
    return render_template("dashboard_prospections.html",prospections=Prospection.query.filter_by(commercial_id=current_user.id).order_by(Prospection.date.desc(),Prospection.id.desc()).all())
