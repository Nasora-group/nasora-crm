    ...
    if existing_values:
        ...
        for value in existing_values:
            if value and value not in active_set:
                choices.append((value, f"{value} (non disponible)")); active_set.add(value)
...
def _set_prospection_choices(form, visit_date=None, existing_client_id=None):
    ...
    if not planned_types: planned_types = [value for value, _label in STRUCTURES]
    form.structure.choices = [(value, value) for value in planned_types]
    clients = Client.query.filter_by(owner_id=current_user.id).order_by(Client.name.asc()).all()
    ...
    if existing_client_id and existing_client_id not in [client.id for client in clients]:
        ...
        if client: choices.append((client.id, f"{client.name} — {client.structure}"))
...
def _invalid_phone(value):
    raw = (value or "").strip().lower(); digits = _normalize_phone(raw)
...
def _find_client_for_prospection(prospection):
    ...
    candidates = Client.query.filter(Client.name.isnot(None)).all(); same_name = [c for c in candidates if _normalize_text(c.name) == normalized_name]
    owned = [c for c in same_name if c.owner_id == current_user.id]
...
def _sync_professional_from_prospection(prospection):
    phone = (prospection.telephone or "").strip(); name = (prospection.nom_client or "").strip(); client = _find_client_for_prospection(prospection); valid_phone = not _invalid_phone(phone)
    if client is None:
        client = Client(name=name, specialty=(prospection.specialite or "").strip() or None, structure=(prospection.structure or "").strip(), phone=phone if valid_phone else None, potential=3, owner_id=current_user.id, last_visit=prospection.date); db.session.add(client); db.session.flush()
    else:
    ...
    products_presented = prospection.produits_presentes or None; products_prescribed = prospection.produits_prescrits or None; report = prospection.profils_prospect or None
    ...
    if existing_visit is not None: return client, existing_visit
    visit = ClientVisit(client_id=client.id, commercial_id=current_user.id, date=prospection.date, products_presented=products_presented, products_prescribed=products_prescribed, report=report); db.session.add(visit); db.session.flush(); return client, visit
...
def _delete_linked_records_for_prospection(prospection):
    ...
    if visit is None: visit = ClientVisit.query.filter_by(client_id=client.id, commercial_id=prospection.commercial_id, date=prospection.date).order_by(ClientVisit.id.desc()).first()
    if visit is not None: db.session.delete(visit); db.session.flush()
    deleted_client = False
    if ClientVisit.query.filter_by(client_id=client.id).count() == 0: db.session.delete(client); db.session.flush(); deleted_client = True
...
def index():
    form = ProspectionForm(); planned_structures = _set_prospection_choices(form, form.date.data); _set_product_choices(form, current_user.project)
    if form.is_submitted():
        planned_structures = _set_prospection_choices(form, form.date.data, form.prospect_id.data); _set_product_choices(form, current_user.project)
        if not form.validate(): flash("Veuillez corriger les champs indiqués.", "error"); return _render_dashboard(form, planned_structures)
        try:
            client = Client.query.filter_by(id=form.prospect_id.data, owner_id=current_user.id).first() if form.prospect_id.data else None
            if client:
                form.nom_client.data = client.name; form.telephone.data = client.phone or form.telephone.data; form.specialite.data = client.specialty or form.specialite.data
            if form.structure.data not in {value for value, _label in form.structure.choices}:
                flash("Cette structure n'est pas disponible pour cette date.", "error"); return _render_dashboard(form, planned_structures)
            prospection = Prospection(commercial_id=current_user.id, date=form.date.data, nom_client=form.nom_client.data.strip(), specialite=form.specialite.data.strip(), structure=form.structure.data.strip(), telephone=form.telephone.data.strip(), profils_prospect=(form.profils_prospect.data or "").strip(), produits_presentes=", ".join(form.produits_presentes.data or []), produits_prescrits=", ".join(form.produits_prescrits.data or [])); db.session.add(prospection); db.session.flush(); client, visit = _sync_professional_from_prospection(prospection); db.session.commit(); flash("Prospection enregistrée avec succès.", "success"); return redirect(url_for("dashboard.index"))
        except Exception:
...
def edit_prospection(prospection_id):
    ...
    existing_presentes = _parse_products(prospection.produits_presentes); existing_prescrits = _parse_products(prospection.produits_prescrits); form = ProspectionForm(obj=prospection); planned_structures = _set_prospection_choices(form, prospection.date); _set_product_choices(form, current_user.project, existing_values=set(existing_presentes) | set(existing_prescrits))
