...
def _duplicate_product(product_model, name, reference, exclude_id=None):
    """Détecte les doublons avant le commit et donne un message exploitable."""
    normalized_name = (name or "").strip().casefold()
    normalized_reference = (reference or "").strip().casefold()
    query = product_model.query
    if exclude_id is not None:
        query = query.filter(product_model.id != exclude_id)
    for product in query.all():
        if normalized_name and (product.name or "").strip().casefold() == normalized_name:
            return "nom"
        if normalized_reference and (product.reference or "").strip().casefold() == normalized_reference:
            return "référence"
    return None
...
def list_products(slug):
    ...
    return render_template("admin_products.html", products=products, supplier=supplier, slug=slug, suppliers=_active_suppliers(), toggle_form=toggle_form)
...
def create_product(slug):
    ...
    if form.validate_on_submit():
        ...
        duplicate = _duplicate_product(product_model, name, reference)
        if duplicate:
            flash(f"Impossible d'ajouter la référence : le {duplicate} « {name if duplicate == 'nom' else reference} » existe déjà dans {supplier['label']}.", "warning")
        else:
            try:
                product = product_model(name=name, reference=reference, default_price=form.default_price.data, is_active=form.is_active.data)
                ...
                db.session.rollback()
                flash("Impossible d'ajouter cette référence : une référence ou un nom identique existe déjà.", "warning")
            except Exception:
                db.session.rollback()
...
def edit_product(slug, product_id):
    ...
    if form.validate_on_submit():
        name = form.name.data.strip()
        reference = form.reference.data.strip() if form.reference.data else None
        duplicate = _duplicate_product(product_model, name, reference, exclude_id=product.id)
        if duplicate:
            flash(f"Modification impossible : le {duplicate} « {name if duplicate == 'nom' else reference} » existe déjà dans {supplier['label']}.", "warning")
        else:
            try:
                product.name = name
                ...
                db.session.rollback()
                flash("Impossible de mettre à jour cette référence : une référence ou un nom identique existe déjà.", "warning")
            except Exception:
                db.session.rollback()
...
def toggle_product(slug, product_id):
    ...
    try:
        ...
        db.session.rollback()
        logger.exception("Erreur lors du changement de statut du produit #%s", product_id)
        flash("Impossible de modifier le statut de la référence.", "error")
        return redirect(url_for("products.list_products", slug=slug))
