...
def _annual_revenue_for_division(division, year):
    ...
    for sale_model in SALE_MODELS:
        amount_expr = func.coalesce(sale_model.quantity, 0) * func.coalesce(sale_model.price, 0)
        value = db.session.query(func.coalesce(func.sum(amount_expr), 0)).filter(sale_model.project == division, func.extract("year", sale_model.date) == year).scalar()
...
def _commercial_sales_visualization(division):
    ...
    for supplier in SUPPLIERS.values():
        ...
        for sale in rows:
            ...
            details_by_month.setdefault(month_key, []).append({"date": sale.date, "supplier": supplier_label, "product": sale.product.name if sale.product else "Produit", "quantity": sale.quantity, "price": float(sale.price or 0), "amount": amount, "commercial": commercial_names.get(sale.commercial_id, "N/A")})
...
def dashboard():
    ...
    filtered_visit_counts = dict(filtered_query.with_entities(Prospection.commercial_id, func.count(Prospection.id)).group_by(Prospection.commercial_id).all())
...
def commercial_detail(username):
    ...
    # Source de vérité des visites : Prospection. Une ligne de Prospection = une visite réelle.
    # Ne pas utiliser ClientVisit, Planning ou des agrégats de visites pour cette rubrique.
    prospection_query = Prospection.query.filter(Prospection.commercial_id == commercial.id)
    total_real_prospections = prospection_query.count()
    page = request.args.get("page", 1, type=int)
    pagination = prospection_query.order_by(Prospection.date.desc()).paginate(page=page, per_page=25, error_out=False)
